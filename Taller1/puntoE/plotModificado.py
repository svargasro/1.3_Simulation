...
axes.plot(x_t, y_t, "-", color="cyan", label=r"$(x',y')_{T}$",linewidth=0.8)
axes.plot(x_s, y_s, "-", color="yellow", label=r"$(x',y')_{S}$")
axes.plot(x_j, y_j, "-", color="black", label=r"$(x',y')_{J}$", linewidth=0.2)

axes.set_xlabel("x", fontsize=12)
axes.set_ylabel("y'", fontsize=12)
axes.legend(loc='upper right')
axes.grid(True, linestyle='--')
axes.set_title("y vs. x'. Órbitas en sistema que no rota con Júpiter.", fontsize=14)
